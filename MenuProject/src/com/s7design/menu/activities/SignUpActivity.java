...
public class SignUpActivity extends BaseActivity {
	...
	private void initViews() {
		...
		mSignUpButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Braintree braintree = Braintree.getInstance(SignUpActivity.this,
						Menu.getInstance().getDataManager().getClientBraintreeToken(SignUpActivity.this));
	...
	private void initClicksInTermsText() {
		...
		ss.setSpan(clickableSpanPrivacyPolicy, (terms_intro.length() + terms_privacy_clickable.length() + terms_and.length() + 1), (terms_intro.length()
				+ terms_privacy_clickable.length() + terms_and.length() + terms_privacy_policy_clickable.length()), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		ss.setSpan(new ForegroundColorSpan(getResources().getColor(R.color.menu_main_orange)), (terms_intro.length() + terms_privacy_clickable.length()
				+ terms_and.length() + 1),
		...
		ss.setSpan(
				...
				Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		ss.setSpan(
				new ForegroundColorSpan(getResources().getColor(R.color.menu_main_orange)),
				(terms_intro.length() + terms_privacy_clickable.length() + terms_and.length() + terms_privacy_policy_clickable.length()
						+ terms_tokenizer.length() + 1),
				(terms_intro.length() + terms_privacy_clickable.length() + terms_and.length() + terms_privacy_policy_clickable.length()
						+ terms_tokenizer.length() + terms_tokenizer_clickable.length() - 1), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
